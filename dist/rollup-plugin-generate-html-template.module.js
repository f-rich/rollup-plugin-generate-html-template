...
process.cwd = function() {
  if (!cwd$1) cwd$1 = origCwd$1.call(process);
  return cwd$1;
};
...
var gracefulFs$1 = createCommonjsModule(function(module) {
  ...
  module.exports = patch(clone_1$1(fs));
  ...
  function patch(fs) {
    ...
    if (process.version.substr(0, 4) === "v0.8") {
      var legStreams = legacyStreams$1(fs);
      ReadStream = legStreams.ReadStream;
      WriteStream = legStreams.WriteStream;
    }
...
var gracefulFs_1 = gracefulFs$1.close;
var gracefulFs_2 = gracefulFs$1.closeSync;
    ...
    async generateBundle(outputOptions, bundleInfo) {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          const targetName = path.basename(target || template); // Add the file suffix if it isn't there.

          const targetFile =
            targetName.indexOf(".html") < 0 ? `${targetName}.html` : targetName; // Read the file

          const buffer = await lib.readFile(template); // Convert buffer to a string and get the </body> index
          ...
          injected = [
            ...
            ...bundleKeys
              ...
              .map(async (
                b // For embedContent option, stuff bundle content into HTML directly,
              ) =>
